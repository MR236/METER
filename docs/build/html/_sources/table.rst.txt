...
To obtain the transition matrices of the multi-state model you should do the following:
    1. Use :py:func:`METER.wrangler.wide_format` to obtain the data in wide format.
    2. Use :py:func:`METER.table.atrisk_and_transitions` to obtain the risk sets and number of transitions made at each time point. 
    3. Use :py:func:`METER.table.transitionprobs_and_samplesizes` to obtain the transition matrices for each time point, and the associated sample sizes. 
